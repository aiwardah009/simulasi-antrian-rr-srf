...
function addCustomer() {
    ...
    if (name && !isNaN(service) && !isNaN(quantumTime) && service > 0 && quantumTime > 0) {
        customers.push({
            name,
            service
        });
...
function runSimulation() {
    ...
    output +=
        "<table><tr><th>Nama Pelanggan</th><th>Waktu Layanan</th><th>Waktu Tunggu</th><th>Waktu Penyelesaian</th></tr>";

    let waitTimes = Array(customers.length).fill(0);
    let completionTimes = Array(customers.length).fill(0);

    if (queueType === "RR") {
        let queue = customers.map(c => ({
            ...c
        })); // Deep copy
        let time = 0;
        let remaining = queue.map(c => c.service);
        ...
        while (completed < n) {
            // Cari pelanggan dengan waktu tersisa terkecil yang belum selesai
            ...
            if (idx === -1) break; // Semua selesai

            // Proses pelanggan ini sampai selesai
            waitTimes[idx] = time;
